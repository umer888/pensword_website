...
global $post, $product;
// product sale label
$product_sale_label_enable = G5Plus_Auteur()->options()->get_product_sale_label_enable();
if ($product->is_on_sale() && $product->get_type() != 'grouped' && 'on' === $product_sale_label_enable) {
	$product_sale_flash_mode = G5Plus_Auteur()->options()->get_product_sale_flash_mode();
	if ($product_sale_flash_mode == 'text') {
		$product_sale_label_text = G5Plus_Auteur()->options()->get_product_sale_label_text();
	} else {
...
$product_featured_label_enable = G5Plus_Auteur()->options()->get_product_featured_label_enable();
if ($product->is_featured() && 'on' === $product_featured_label_enable) {
	$product_featured_label_text = G5Plus_Auteur()->options()->get_product_featured_label_text();
...
// product new label
$product_new_label_enable = G5Plus_Auteur()->options()->get_product_new_label_enable();
if ('on' === $product_new_label_enable) {
    $product_new_label_since = G5Plus_Auteur()->options()->get_product_new_label_since();
    ...
    if($diff->days <= intval($product_new_label_since)) {
        $product_new_label_text = G5Plus_Auteur()->options()->get_product_new_label_text();
