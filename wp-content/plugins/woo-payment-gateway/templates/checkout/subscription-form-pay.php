...
<form id="order_review" method="post">
	...
	<table class="shop_table">
		...
		<tfoot>
			...
			<?php if ( $totals ) : ?>
				<tr><th colspan="3" class="wcs-braintree-recurring-totals"><?php printf(__('Recurring Totals: #%1$s', 'woo-payment-gateway'), $subscription->get_id())?></th></tr>
				<?php foreach ( $totals as $id => $total ) : ?>
					<tr class="wcs-braintree-total-row <?php echo $id?>">
						<td scope="row" colspan="2"><?php echo $total['label']; ?></th><?php // @codingStandardsIgnoreLine ?>
						<td class="product-total"><?php echo $total['value']; ?></td><?php // @codingStandardsIgnoreLine ?>
					</tr>
				<?php endforeach; ?>
					<tr class="wcs-braintree-total-row">
						<td scope="row" colspan="2"><?php _e('First Renewal', 'woo-payment-gateway')?>:
