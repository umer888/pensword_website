...
class WebhookTestingGateway
{
    ...
    public function sampleNotification($kind, $id, $sourceMerchantId = null, $xml = null)
    {
        $xml = self::_sampleXml($kind, $id, $sourceMerchantId, $xml);
    ...
    private static function _sampleXml($kind, $id, $sourceMerchantId, $xml = null)
    {
        switch ($kind) {
            ...
            case WebhookNotification::LOCAL_PAYMENT_COMPLETED:
                ...
                break;
            default:
                $subjectXml = self::_subscriptionSampleXml($id, $xml);
                break;
        }
    ...
    private static function _subscriptionSampleXml($id, $xml = null)
    {
    	if(null !== $xml){
    		return $xml;
    	}
    ...
    private static function _subscriptionChargedSuccessfullySampleXml($id)
    {
        ...
        <subscription>
            ...
            <next-billing-date type=\"date\">2019-05-11</next-billing-date>
            <transactions type=\"array\">
                <transaction>
                    <id>{$id}" . rand(0, 10000) . "</id>
                    <status>submitted_for_settlement</status>
                    <amount>49.99</amount>
                    <created-at type=\"datetime\">2019-03-11T04:48:38Z</created-at>
                </transaction>
            </transactions>
            <add_ons type=\"array\">
            </add_ons>
