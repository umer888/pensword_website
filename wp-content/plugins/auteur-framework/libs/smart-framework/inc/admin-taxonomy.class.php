			...
			foreach ($custom_tax as $tax => $args) {
				...
				$default = array(
					...
					'labels'       => array(
						...
						'menu_name'                  => $label,
						'search_items'               => sprintf(esc_html__('Search %s', 'auteur-framework'), $label),
						'popular_items'              => sprintf(esc_html__('Popular %s', 'auteur-framework'), $label),
						'all_items'                  => sprintf(esc_html__('All %s', 'auteur-framework'), $label),
						'parent_item'                => sprintf(esc_html__('Parent %s', 'auteur-framework'), $singular_name),
						'parent_item_colon'          => sprintf(esc_html__('Parent %s:', 'auteur-framework'), $singular_name),
						'edit_item'                  => sprintf(esc_html__('Edit %s', 'auteur-framework'), $singular_name),
						'view_item'                  => sprintf(esc_html__('View %s', 'auteur-framework'), $singular_name),
						'update_item'                => sprintf(esc_html__('Update %s', 'auteur-framework'), $singular_name),
						'add_new_item'               => sprintf(esc_html__('Add New %s', 'auteur-framework'), $singular_name),
						'new_item_name'              => sprintf(esc_html__('New %s New', 'auteur-framework'), $singular_name),
						'separate_items_with_commas' => sprintf(esc_html__('Separate %s with commas', 'auteur-framework'), strtolower($label)),
						'add_or_remove_items'        => sprintf(esc_html__('Add or remove %s', 'auteur-framework'), strtolower($label)),
						'choose_from_most_used'      => sprintf(esc_html__('Choose from the most used %s', 'auteur-framework'), strtolower($label)),
						'not_found'                  => sprintf(esc_html__('No %s found.', 'auteur-framework'), strtolower($label)),
						'no_terms'                   => sprintf(esc_html__('No %s', 'auteur-framework'), strtolower($label)),
						'items_list_navigation'      => sprintf(esc_html__('%s list navigation', 'auteur-framework'), $label),
						'items_list'                 => sprintf(esc_html__('%s list', 'auteur-framework'), $label),
					)
				);
		...
		public function registerTermMeta() {
			...
			foreach ($meta_configs as $meta_id => $config) {
				...
				foreach ($taxonomies as $taxonomy) {
					...
					add_action( 'edited_' . $taxonomy, array($this, 'saveTermMeta'), $taxonomy_priority, 2 );
				}
			}
		}
