	...
	class GSF_Field_File extends GSF_Field
	{
		function enqueue() {
			...
			wp_localize_script(GSF()->assetsHandle('field_file'), 'GSF_FILE_FIELD_META', array(
				'title'   => esc_html__('Select File', 'auteur-framework'),
				'button'  => esc_html__('Use these files', 'auteur-framework')
			));
		}
		...
		function renderContent()
		{
			...
			$remove_text = esc_html__('Remove','auteur-framework');
            $attributes = array();
			if (isset($this->_setting['lib_filter']) && !empty($this->_setting['lib_filter'])) {
                $attributes[] = sprintf('data-lib-filter="%s"', esc_attr($this->_setting['lib_filter']));
			}
			?>
			<div class="gsf-field-file-inner" data-remove-text="<?php echo esc_attr($remove_text); ?>" <?php echo join(' ', $attributes); ?>>
				...
				<div class="gsf-file-add">
					<button class="button" type="button"><?php esc_html_e('+ Add File','auteur-framework'); ?></button>
				</div>
			</div>
			<?php
		}
