			...
			<div class="gsf-field-image-inner gsf-clearfix">
				...
				<div class="gsf-image-info">
					<input data-field-control="" type="text"
					       class="gsf-image-url" placeholder="<?php esc_attr_e('No image', 'auteur-framework'); ?>"
					       name="<?php $this->theInputName(); ?>[url]"
					       value="<?php echo esc_url($field_value['url']); ?>"/>
					<button type="button" class="button gsf-image-choose-image"><?php esc_html_e('Choose Image', 'auteur-framework'); ?></button>
					<button type="button"
					        class="button gsf-image-remove"><?php esc_html_e('Remove', 'auteur-framework'); ?></button>
				</div>
			</div>
		<?php
		}
