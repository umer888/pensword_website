...
if (!class_exists('XMenuWalker')) {
	class XMenuWalker extends Walker_Nav_Menu {
		 ...
		 *
		 * @since 3.0.0menu-submenu-position
		 * @access public
		 * @todo Decouple this.
		 * @var array
		 *
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			 ...
			 */
			if (($depth > 0) && ($item->object === 'xmenu_mega')) {
				//$postMega = get_post($item->object_id);
				$content = get_post_field('post_content', $item->object_id);
				...
				if (is_array($xmenu_meta)) {
					if (isset($xmenu_meta['menu-item-featured-style']) && ($xmenu_meta['menu-item-featured-style'] != '')) {
						...
						switch ($xmenu_meta['menu-item-featured-style']) {
							case 'hot': {
								$menu_featured_text = esc_html__('Hot','auteur-framework');
								break;
							}
							case 'new': {
								$menu_featured_text = esc_html__('New','auteur-framework');
								break;
							}
							default: {
								$menu_featured_text = esc_html__('Featured','auteur-framework');
							}
						}
