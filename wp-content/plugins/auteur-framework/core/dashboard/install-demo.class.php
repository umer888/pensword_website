...
if (!class_exists('G5P_Dashboard_Install_Demo')) {
	class G5P_Dashboard_Install_Demo
	{
		...
		{
			add_action('admin_enqueue_scripts', array($this, 'enqueue_resource'));
		}

		public function enqueue_resource() {
			wp_enqueue_style(G5P()->assetsHandle('install_demo_data'), G5P()->helper()->getAssetUrl('core/dashboard/install-demo/assets/css/style.min.css'),array(),G5P()->pluginVer());
			wp_enqueue_script(G5P()->assetsHandle('install_demo_data'), G5P()->helper()->getAssetUrl('core/dashboard/install-demo/assets/js/app.min.js'), array('jquery'), G5P()->pluginVer(), true);
		...
		public function install() {
			...
			if (!(isset($_REQUEST['security']) && current_user_can( 'manage_options' )) )
			{
				...
				$data_response = array(
					'code' => 'error',
					'message' => esc_html__("Permission error!",'auteur-framework')
				);
			...
			if (!G5P()->loadFile(G5P()->pluginDir('core/dashboard/install-demo/wordpress-importer.php'))) {
				$importer_error = true;
			}

			if (!G5P()->loadFile(G5P()->pluginDir('core/dashboard/install-demo/g5plus_import_class.php'))) {
				$importer_error = true;
			}
			...
			if ($importer_error !== false) {
				...
				$data_response = array(
					'code' => 'fileNotFound',
					'message' => esc_html__("The Auto importing script could not be loaded. please use the wordpress importer and import the XML file that is located in your themes folder manually.",'auteur-framework')
				);
				echo json_encode($data_response);
				die();
			}
				...
				switch (trim($type)) {
					...
					case 'import-setting':
						...
						try {
							...
							$data_response = array(
								'code' => 'error',
								'message' => esc_html__('Import Error','auteur-framework')
							);
							echo json_encode($data_response);
							die();
						}
