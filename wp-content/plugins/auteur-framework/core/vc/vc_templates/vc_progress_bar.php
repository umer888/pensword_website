 ...
 */
$layout_style = $title = $values = $units = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = '';
$output = '';
...
$class_to_filter = 'vc_progress_bar wpb_content_element '.$layout_style.'';
...
foreach ( $values as $data ) {
	...
	if ( isset( $data['color'] ) && 'custom' === $data['color'] && isset( $data['customcolor'] ) ) {
		$new_line['bgcolor'] = ' style="background-color: ' . esc_attr( $data['customcolor'] ) . ';"';
	}
	if ( isset( $data['color'] ) && 'custom' === $data['color'] && isset( $data['customtxtcolor'] ) ) {
...
foreach ( $graph_lines_data as $line ) {
	...
	if($layout_style != ''){
		if(!empty($line['label'])){
			$output .= '<span class="progress-bar-title"' . $line['txtcolor'] . '>' . $line['label'] . '</span>';
		}
