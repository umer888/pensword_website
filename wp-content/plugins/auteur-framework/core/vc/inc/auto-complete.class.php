...
if (!class_exists('G5P_Vc_Auto_Complete')) {
	class G5P_Vc_Auto_Complete {
		...
		public function define_filter(){
            ...
            add_filter( 'vc_autocomplete_gsf_product_singular_product_id_callback', array(&$this,'product_search',), 10, 1 ); // Get suggestion(find). Must return an array
            add_filter( 'vc_autocomplete_gsf_product_singular_product_id_render', array(&$this,'post_render',), 10, 1 ); // Render exact product. Must return an array (label,value)
            ...
            add_filter( 'vc_autocomplete_gsf_portfolio_singular_portfolio_ids_callback', array(&$this,'portfolio_search',), 10, 1 ); // Get suggestion(find). Must return an array
            add_filter( 'vc_autocomplete_gsf_portfolio_singular_portfolio_ids_render', array(&$this,'post_render',), 10, 1 ); // Render exact product. Must return an array (label

            add_filter( 'vc_autocomplete_gsf_events_event_ids_callback', array(&$this,'event_search',), 10, 1 ); // Get suggestion(find). Must return an array
            add_filter( 'vc_autocomplete_gsf_events_event_ids_render', array(&$this,'post_render',), 10, 1 ); // Render exact product. Must return an array (label,value)
        ...
        public function event_search( $search_string ) {
            $query = $search_string;
            $data = array();
            $post_type = (class_exists('Tribe__Events__Main') ? Tribe__Events__Main::POSTTYPE : '');
            $args = array(
                's' => $query,
                'post_type' => $post_type,
            );
