...
if (!class_exists('G5P_Widget_Price_Filter')) {
    class G5P_Widget_Price_Filter extends GSF_Widget
    {
        public function __construct()
        {
            ...
            $this->widget_description = __('Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'auteur-framework');
            $this->widget_id = 'gsf-price-filter';
            $this->widget_name = __('G5Plus: Price Filter List', 'auteur-framework');
            $this->settings = array(
                'fields' => array(
                    array(
                        'id'      => 'title',
                        ...
                        'type'    => 'text',
                        'default' => __('Filter by price', 'auteur-framework')
                    ),
                    array(
                        'id' => 'price_range_size',
                        'type'       => 'text',
                        ...
                        'default' => 50,
                        'title' => __('Price range size', 'auteur-framework')
                    ),
                    array(
                        'id' => 'max_price_ranges',
                        'type'       => 'text',
                        ...
                        'default' => 5,
                        'title' => __('Max price ranges', 'auteur-framework')
                    ),
                    array(
                        'id' => 'hide_empty_ranges',
                        'type' => 'switch',
                        'default' => 'on',
                        'title' => __('Hide empty price ranges', 'auteur-framework')
                    )
                )
            );
        ...
        function widget($args, $instance)
        {
            ...
            if ($qs_count > 0) {
                ...
                foreach ($_GET as $key => $value) {
                    if(in_array($key, array('min_price', 'max_price'))) continue;
                    $i++;
                    $link .= $key . '=' . $value;
                    if ($i != $qs_count) {
                        $link .= '&';
            ...
            if (strlen($min_price) > 0) {
                $output .= '<li><a class="gsf-link transition03" href="' . esc_url($link) . '">' . esc_html__('All', 'auteur-framework') . '</a></li>';
            } else {
                $output .= '<li class="active"><span>' . esc_html__('All', 'auteur-framework') . '</span></li>';
            }
